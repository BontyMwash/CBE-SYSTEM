...
const UI = {
  ...
  _stampPdfFooter(pdf, footerOpts) {
    ...
    const topHeader = useDefault ? '' : (footerOpts.header || '');
    ...
    for (let i = 1; i <= pageCount; i++) {
      pdf.setPage(i);
      // Optional repeating report header.  This is deliberately drawn
      // inside the existing top margin so it appears on EVERY PDF page
      // without consuming or overlapping the broadsheet table itself.
      if (topHeader) {
        pdf.setFillColor(255, 255, 255);
        pdf.rect(0, 0, pw, UI._PDF_MARGIN_MM, 'F');
        pdf.setDrawColor(200);
        pdf.setLineWidth(0.2);
        pdf.line(UI._PDF_MARGIN_MM, UI._PDF_MARGIN_MM - 1.2, pw - UI._PDF_MARGIN_MM, UI._PDF_MARGIN_MM - 1.2);
        pdf.setFont(undefined, 'bold');
        pdf.setFontSize(7.2);
        pdf.setTextColor(70, 80, 95);
        const maxHeaderWidth = pw - (UI._PDF_MARGIN_MM * 2);
        const headerLines = typeof pdf.splitTextToSize === 'function'
          ? pdf.splitTextToSize(topHeader, maxHeaderWidth)
          : [topHeader];
        pdf.text(headerLines.slice(0, 1), UI._PDF_MARGIN_MM, 4.8, { align: 'left' });
